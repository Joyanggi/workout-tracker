 ...
 * 소리 진단 스냅샷.
 *
 * 실사용에서 **모든 소리가 안 들린다**는 보고가 있었고, 개발 브라우저에서는
 * `navigator.audioSession`이 아예 없어(그리고 창이 항상 `hidden`이라 §5.2 가드에 막혀)
 * 재현이 불가능하다. 그래서 **실기기가 직접 사실을 보고**하게 만든다 —
 * 짐작으로 고치는 것을 이 프로젝트는 금지한다.
 */
export interface AudioDiagnostics {
  sessionSupported: boolean
  /** 지금 지정돼 있는 세션 종류. 미지원이면 null */
  sessionType: string | null
  ctxExists: boolean
  /** running / suspended / interrupted — suspended면 제스처 밖에서 못 깨운다 */
  ctxState: string | null
  sampleRate: number | null
  /** 시간이 흐르고 있는가 (suspended면 멈춘다) */
...
export function audioDiagnostics(): AudioDiagnostics {
  const nav = navigator as AudioSessionNavigator
  return {
    sessionSupported: nav.audioSession !== undefined,
...
/**
 * 세션 종류를 바꿔 한 번 재생해 본다 (진단 전용).
 *
 * **결정적 실험**: `ambient`로만 안 들리고 `playback`으로는 들리면 원인은 무음 스위치나
 * 출력 경로다 (ambient는 무음 스위치를 존중한다). 둘 다 안 들리면 컨텍스트 쪽이다.
 *
 * 끝나면 **반드시 ambient로 되돌린다** — playback으로 남으면 다른 앱 음악을 끊는다(G8 위반).
 */
export function playWithSessionType(type: string, spec: ToneSpec): AudioDiagnostics {
  const nav = navigator as AudioSessionNavigator
  if (nav.audioSession) {
    try {
  ...
  tone(spec)
  const after = audioDiagnostics()
  // 재생이 끝난 뒤 원복 (재생 중에 바꾸면 실험이 무의미해진다)
  const restoreAfterMs = ((spec.delay ?? 0) + spec.duration) * 1000 + 800
  window.setTimeout(configureAudioSession, restoreAfterMs)
  return after
}

/** 진단용 확인음 — 놓칠 수 없게 길고 크다 ("들렸나요?"가 깔끔한 예/아니오가 되도록) */
export const PROBE: ToneSpec = { freq: 880, duration: 1, gain: 0.3 }
