...
export async function createBackup(now: Date = new Date()): Promise<BackupFile> {
  const [routines, exercises, sessions, settings] = await Promise.all([
    db.routines.toArray(),
 ...
 */
export function parseBackup(text: string): { file: BackupFile } | { problems: string[] } {
  let raw: unknown
  try {
    raw = JSON.parse(text)
  } catch (err) {
    return { problems: [`JSON 파싱 실패: ${err instanceof Error ? err.message : String(err)}`] }
  }
  ...
  for (const key of ['routines', 'exercises', 'sessions', 'settings'] as const) {
...
export async function restoreBackup(file: BackupFile): Promise<void> {
  await db.transaction('rw', db.routines, db.exercises, db.sessions, db.settings, async () => {
    await Promise.all([
      db.routines.clear(),
      db.exercises.clear(),
      db.sessions.clear(),
    ...
    await Promise.all([
      ...
      db.settings.bulkPut(file.settings.filter((r) => !isSecretSettingKey(r.key))),
    ])
  })
}
