 ...
 * 페이즈 소리 — **경계 단발음** (CC7-R).
 *
 * v1.8에서 연속 글라이드로 바꿨다가 **실기기 청감으로 기각됐다**: "사이렌 같다".
 * 후보 5종(글라이드 / 하프 롤 / 호흡 패드 / 밝기 스윕 / 경계음+초읽기 틱)을 들려준 결과
 * "글라이드가 없었던 버전이 최선"이 판정이어서 v1.7까지의 방식으로 되돌렸다.
 *
 * 아래는 되돌린 뒤 지켜야 하는 성질이다 — 게인은 CC8 비율(+50%)에 맞춰 올렸지만
 * **대역·길이는 청감이 승인한 값 그대로**여야 한다.
 */
describe('페이즈 경계음 (CC7-R)', () => {
  ...
  it('수축이 이완보다 높은 음이다 (상행 vs 하행)', () => {
    expect(phaseTone(TEMPO.B[0]).freq).toBeGreaterThan(phaseTone(TEMPO.B[2]).freq)
  })
  ...
  it('수축 톤이 페이즈보다 길어지지 않는다', () => {
    expect(phaseTone({ kind: 'concentric', seconds: 0.2 }).duration).toBeLessThanOrEqual(0.2)
  })

  it('게인이 CC8 상향 비율(+50%)에 맞춰 올랐다', () => {
    /*
      다른 소리(틱 0.2→0.3, 차임 0.26→0.36, 큐 0.22→0.3)만 커지고 템포 톤만 옛 크기면
      상대 밸런스가 깨진다. 구 값 0.18 / 0.14 → 0.27 / 0.21.
    */
    expect(phaseTone({ kind: 'concentric', seconds: 1 }).gain).toBeCloseTo(0.27, 5)
    expect(phaseTone({ kind: 'eccentric', seconds: 2 }).gain).toBeCloseTo(0.21, 5)
    expect(phaseTone({ kind: 'squeeze', seconds: 1 }).gain).toBeCloseTo(0.21, 5)
  })

  it('세 페이즈 계열이 서로 다른 높이다 — 구분은 피치와 길이로 한다', () => {
    // 램프로 이어 붙이지 않고 높이로 구분한다 (이어 붙이는 순간 사이렌이 된다)
    const freqs = new Set([
      phaseTone({ kind: 'concentric', seconds: 1 }).freq,
      phaseTone({ kind: 'eccentric', seconds: 2 }).freq,
      phaseTone({ kind: 'squeeze', seconds: 1 }).freq,
    ])
    expect(freqs.size).toBe(3)
  })

  it('템포 톤이 틱·차임 대역과 겹치지 않는다', () => {
    // 틱 784Hz — 템포 톤이 그보다 낮아야 카운트인과 페이즈 소리가 구분된다
    for (const kind of ['concentric', 'eccentric', 'squeeze'] as const) {
      expect(phaseTone({ kind, seconds: 1 }).freq, kind).toBeLessThan(TICK.freq)
    }
  })
})
...
describe('가이드 종료 라벨의 신뢰도 표시 (V1)', () => {
  ...
  /*
    라벨 **문구**가 아니라 두 성질을 단정한다 — Z2에서 문구가 바뀌었을 때
    (`회로 기록` → `회 기록 · 휴식 시작`) 이 테스트가 문구 때문에 깨졌다.
    지켜야 하는 것은 "약이 붙는다"와 "값이 보인다"이고 어순이 아니다.
  */
  ...
  it('넣을 값을 버튼에 미리 보여준다 (방어선 — 리뷰어 지시)', () => {
    ...
    expect(sheet()).toMatch(/\$\{rep\.reps\}회/)
  })

  it('자동 종료는 별도 prop이다 (onComplete) — 값의 출처가 구분된다', () => {
    /*
      Z2에서 두 경로의 **동작**은 같아졌다 (기록·체크·휴식). 그래도 prop을 둘로 남기는
      이유는 값의 신뢰도가 다르기 때문이다 — 자동은 repMax와 정확히 일치하고 수동은 추정이다.
      "약" 라벨은 수동 버튼에만 붙는다.
    */
