...
> **구현 주석 (마일스톤 2, 2026-07-31).** 위 표 2행(이력 D1 → 제안 D2)은 위에 적힌 점수식만으로는
> 성립하지 않는다. 원식으로는 D4=15.40 > D2=14.80이 되어 D4가 뽑히고, 아래의 귀결
> (D1→D2→D4→D3)도 깨진다. 3행은 문서가 계산을 명시한 유일한 행이고(10.5 / 5.55) 구현이 그 값을
> 정확히 재현하므로, 식이 아니라 2행의 산술이 누락된 것으로 판단했다.
>
> 해결: **이번 주 노출 0회인 부위의 기여에 ×1.5**를 얹었다. §7의 "부위별 주 2회 노출 우선 —
> 볼륨을 맞춰도 주 2회 > 주 1회"를 점수로 옮긴 것이다. 원식은 부위별 *남은 세트 수*만 보므로
> 빈도 우선을 표현하지 못한다(광배 0/16의 첫 6세트와 측면어깨 7/11의 남은 4세트를 동급 취급).
> 배수의 유효 구간은 2행 조건 `b > 4.0/3.4 = 1.176`과 3행 조건 `b < 10.5/5.55 = 1.892`의
> 교집합이며, 경계에서 가장 먼 1.5를 채택했다. 표 4행 전부와 이 부등식이 `suggestNextDay.test.ts`에
> 고정돼 있다.
>
> 또한 "주요 부위 겹침"의 정의가 명세에 없어 **그 Day 총 세트의 25% 이상**으로 정했다.
> 느슨하게 잡으면 D1과 D2가 `팔`로 겹쳐 거의 모든 Day가 같이 감쇠되고 회복 제약이 무의미해진다.
...
- 홈 화면에서 제안 Day를 크게 표시하되, 카드 탭 또는 **[변경] 버튼**으로 **모든 Day + 하한 모드(Push 축소/Pull 축소) 선택 시트**가 열림 (요구사항: 잘못된 경우 변경 가능)
- **시트는 선택만 바꾸고, 시작은 항상 시작 버튼이다** (AA1·AA2 개정). 초기 구현은 시트에서 행을 탭하는 순간 세션을 만들었다 — 제안 Day는 2단(카드 → 시작 버튼)인데 다른 Day는 1단이어서 같은 결정의 무게가 경로에 따라 달라지고, 둘러보기(Day별 종목·세트 비교)가 불가능했다. 선택 상태는 **저장하지 않는다** (탭을 옮기면 제안으로 복귀 — 다음 날 어제의 수동 선택이 남는 것이 더 나쁜 실패다)
- 직접 선택 상태에서는 제안 근거 자리에 "직접 선택 — 제안은 {제안 Day}"를 표시 (제안이 무시된 게 아니라 보류된 것)
...
- ~~드래그 핸들로 리스트 재정렬~~ — **v1에서 의도적으로 제외.** 수행 순서 분석은
  `performedOrder`가 담당하고(첫 세트 체크 시각 순 자동 부여), 카드는 아무 순서로나
  탭해서 시작할 수 있다. 드래그는 시각적 정리 이상의 가치가 없어 iOS 터치 드래그
  구현 복잡도 대비 효용이 미달이다.
  - **v1.1로 연기.** 분석은 전부 `performedOrder` 기준이므로 기능적 손실이 없고, 카드를 아무 순서로나 펼쳐 입력할 수 있어 실사용에 지장이 없다
