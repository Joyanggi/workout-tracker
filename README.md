...
| **세션** | 접힘/펼침 운동 카드, 종목별 자극 팁, 머신 세팅 메모, 종목별 무게 단위, 롱프레스 가속 스테퍼, 세트 체크 → 휴식 타이머 자동 시작, 워밍업 세트, 대체운동 + 무게 환산, 감각 점수, 보상작용 반복 경고, 수행 순서 자동 기록 |
| **기록** | 월 달력(불꽃), 세션 상세 + 편집, 종목별 히스토리, Day 재매핑 |
| **분석** | 종목별 추이(무게×횟수 / 볼륨 / e1RM, B그룹은 Phase 2부터), 감각 점수 4주 히트맵, 주간 수행 횟수, 대체 수행 빈도 |
| **설정** | Phase 전환 + 조건 체크리스트, 루틴 상세·JSON 교체, Markdown/JSON 내보내기, JSON 복원, Gist 백업 |
...
다섯 개입니다 (스키마 v2 — `exerciseNotes` 행이 머신 세팅 메모와 무게 단위를 함께 담습니다).
...
- 테스트: Vitest (알고리즘 단위 테스트 321개, CI에서 배포 전 실행)
