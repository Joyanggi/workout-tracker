...
| 탭 | 내용 |
|---|---|
| **홈** | Day 자동 제안 + 근거, 주간 도트, 부위별 볼륨 바, Phase 0 진행률, 디로드 카운터, 복귀·디로드·증량·백업 배너 |
| **세션** | 접힘/펼침 운동 카드, 롱프레스 가속 스테퍼, 세트 체크 → 휴식 타이머 자동 시작, 감각 점수, 보상작용 체크리스트, 수행 순서 자동 기록 |
| **기록** | 월 달력(불꽃), 세션 상세 + 편집, 종목별 히스토리, Day 재매핑 |
| **분석** | A그룹 추이(무게×횟수 / 볼륨 / e1RM), 감각 점수 4주 히트맵, 주간 수행 횟수 |
| **설정** | Phase 전환, 루틴 상세·JSON 교체, Markdown/JSON 내보내기, JSON 복원, Gist 백업 |
